...
auc(Swiss_ST_keras_caox_roc_oversample)
ci.auc(Swiss_ST_keras_caox_roc_oversample)
auc(Swiss_ST_keras_cap_roc_oversample)
ci.auc(Swiss_ST_keras_cap_roc_oversample)
auc(Swiss_ST_keras_urate_roc_oversample)
ci.auc(Swiss_ST_keras_urate_roc_oversample)

Swiss_ST_keras_prediction_all_one_oversample <-
  Swiss_ST_keras_prediction2_oversample %>% mutate(Stone_type = case_when(CaOx > 0.4 ~ "CaOx",
                                                               CaP >
                                                                 0.4 ~ "CaP",
                                                               Urate >
                                                                 0.4 ~ "Urate"))
Swiss_ST_keras_prediction_all_one_oversample$Stone_type <- factor(Swiss_ST_keras_prediction_all_one_oversample$Stone_type,
                                                       levels = c("CaOx",
                                                                  "CaP",
                                                                  "Urate"))
Swiss_ST_test_coded_outcome_all_one_oversample <-
  Swiss_ST_test_coded_outcome3 %>% mutate(Stone_type = case_when(CaOx == 1 ~ "CaOx",
                                                                 CaP ==
                                                                   1 ~ "CaP",
                                                                 Urate ==
                                                                   1 ~ "Urate"))
Swiss_ST_test_coded_outcome_all_one_oversample$Stone_type <- factor(Swiss_ST_test_coded_outcome_all_one_oversample$Stone_type,
                                                         levels = c("CaOx",
                                                                    "CaP",
                                                                    "Urate"))
...
Swiss_ST_keras_tb_oversample <-
  table(ref = Swiss_ST_test_coded_outcome_all_one_oversample$Stone_type, 
        pred = Swiss_ST_keras_prediction_all_one_oversample$Stone_type)
confusionMatrix(Swiss_ST_keras_tb_oversample)
Swiss_ST_keras_multiclass_roc_oversample <- multiclass.roc(Swiss_ST_test_coded_outcome_all_one_oversample$Stone_type,
                                                as.data.frame(Swiss_ST_keras_prediction2_oversample))
